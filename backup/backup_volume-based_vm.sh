...
VM_ID=`nova show --minimal $VM_NAME | grep " id " | awk '{print $4}'`
echo "VM ID: $VM_ID"

# backup all volumes which are attached to VM
for VMVOL_ID in `cinder list | grep $VM_ID | awk '{print $2}'`; do
  echo
  echo "VM volume ID: $VMVOL_ID"
  VMVOL_NAME=`cinder list | grep $VMVOL_ID | awk '{print $6}'`
  echo "VM volume name: $VMVOL_NAME"
  VMVOL_SIZE=`cinder list | grep $VMVOL_ID | awk '{print $8}'`
  echo "VM volume size: $VMVOL_SIZE GB"
  SNAPSHOT_NAME="${NOW}_${VMVOL_NAME}-snapshot"
  echo "snapshot name: $SNAPSHOT_NAME"
  BACKUPVOL_NAME="${NOW}_${VMVOL_NAME}-backupvol"
  echo "backup volume name: $BACKUPVOL_NAME"
  BACKUPIMAGE_BASE="${VMVOL_NAME}-backupimage"
  BACKUPIMAGE_NAME="${NOW}_${BACKUPIMAGE_BASE}"
  ...
  echo
  echo cinder create --snapshot-id $SNAPSHOT_ID --display-name $BACKUPVOL_NAME $VMVOL_SIZE
  cinder create --snapshot-id $SNAPSHOT_ID --display-name $BACKUPVOL_NAME $VMVOL_SIZE
  ...
  if [ "$IMPORT_ON_REMOTE" == "1" ]; then
    ...
    echo "ssh ${REMOTE_SERVER} \"source /root/openrc && export OS_TENANT_NAME=$REMOTE_TENANT_NAME && glance image-create \
      ...
      --disk-format qcow2 --min-disk $VMVOL_SIZE --min-ram 256 \
    ...
    NUM_IMAGES=`ssh ${REMOTE_SERVER} "source /root/openrc && export OS_TENANT_NAME=$REMOTE_TENANT_NAME \
      && glance image-list | grep $BACKUPIMAGE_BASE" | awk '{print $4}' | wc -l`
    if [ "$NUM_IMAGES" -gt "3" ]; then
      ...
      echo "ssh ${REMOTE_SERVER} \"source /root/openrc && export OS_TENANT_NAME=$REMOTE_TENANT_NAME \
        && glance image-list | grep $BACKUPIMAGE_BASE | awk '{print \\$4}' | head -n $(($NUM_IMAGES - 3)) | \
  ...
  NUM_IMAGES=`glance image-list | grep $BACKUPIMAGE_BASE | awk '{print $4}' | wc -l`
  if [ "$NUM_IMAGES" -gt "3" ]; then
    echo
    echo "There are $NUM_IMAGES local backup images. Will only keep the 3 most recent ones."
    echo "glance image-list | grep $BACKUPIMAGE_BASE | awk '{print \$4}' | head -n $(($NUM_IMAGES - 3)) | xargs -L 1 glance image-delete"
    glance image-list | grep $BACKUPIMAGE_BASE | awk '{print $4}' | head -n $(($NUM_IMAGES - 3)) | xargs -L 1 glance image-delete
  fi
